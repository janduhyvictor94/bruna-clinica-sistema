  ...
  return (
    <div className="space-y-6">
                      ...
                      <CardContent className="p-3 sm:p-4">
                        <div className="flex items-start justify-between mb-2 sm:mb-3">
                          <div className="flex items-center gap-2">
                            <div className={`p-1.5 sm:p-2 rounded-lg ${isCompleted ? 'bg-emerald-100' : 'bg-stone-100'}`}>
                              {isCompleted ? <Check className="w-3 h-3 sm:w-4 sm:h-4 text-emerald-600" /> : <Target className="w-3 h-3 sm:w-4 sm:h-4 text-stone-600" />}
                            </div>
                          ...
                          <div className="flex gap-0.5 sm:gap-1 flex-shrink-0">
                            <Button variant="ghost" size="icon" className="h-7 w-7 sm:h-8 sm:w-8" onClick={() => setEditingGoal(goal)}><Edit2 className="w-3 h-3 sm:w-4 sm:h-4" /></Button>
                            <Button variant="ghost" size="icon" className="h-7 w-7 sm:h-8 sm:w-8 text-rose-600 hover:text-rose-700" onClick={() => setDeleteGoal(goal)}><Trash2 className="w-3 h-3 sm:w-4 sm:h-4" /></Button>
                          </div>
                        </div>
                        ...
                        <div className="space-y-1.5 sm:space-y-2">
                          <div className="flex justify-between text-xs sm:text-sm">
                            ...
                            <span className={`font-medium ${isCompleted ? 'text-emerald-600' : 'text-stone-700'}`}>{progress.toFixed(0)}%</span>
                          </div>
                          <Progress value={progress} className="h-1.5 sm:h-2" />
                          <div className="flex justify-between text-[10px] sm:text-xs text-stone-400">
                            <span>{goal.type === 'Faturamento' ? `R$ ${currentValue.toLocaleString('pt-BR', { minimumFractionDigits: 0, maximumFractionDigits: 0 })}` : currentValue}</span>
                            <span>Meta: {goal.type === 'Faturamento' ? `R$ ${goal.target_value.toLocaleString('pt-BR', { minimumFractionDigits: 0, maximumFractionDigits: 0 })}` : goal.target_value}</span>
                          </div>
                        </div>
                      </CardContent>
                    </Card>
      ...
      <AlertDialog open={!!deleteGoal} onOpenChange={() => setDeleteGoal(null)}>
        <AlertDialogContent>
          <AlertDialogHeader><AlertDialogTitle>Excluir Meta</AlertDialogTitle><AlertDialogDescription>Tem certeza que deseja excluir esta meta? Esta ação não pode ser desfeita.</AlertDialogDescription></AlertDialogHeader>
          <AlertDialogFooter><AlertDialogCancel>Cancelar</AlertDialogCancel><AlertDialogAction onClick={() => deleteMutation.mutate(deleteGoal.id)} className="bg-rose-600 hover:bg-rose-700">Excluir</AlertDialogAction></AlertDialogFooter>
        </AlertDialogContent>
      </AlertDialog>
    </div>
  );
