...
export default function Patients() {
  ...
  const { data: patients = [], isLoading } = useQuery({
    queryKey: ['patients'],
    queryFn: async () => {
      // ALTERAÇÃO AQUI: Buscamos também a relação de appointments (data e status)
      // para poder listar múltiplos retornos no card
  ...
  const createMutation = useMutation({
    mutationFn: async (data) => {
      ...
      // Removemos next_return_date manual pois o sistema calcula sozinho agora
  ...
  const updateMutation = useMutation({
    mutationFn: async ({ id, data }) => {
      const cleanData = { ...data };
      if (!cleanData.birth_date) cleanData.birth_date = null;
      delete cleanData.next_return_date; // Evita sobrescrever a inteligência do banco
  ...
  const deleteMutation = useMutation({
    mutationFn: async (id) => { 
        // Primeiro apaga dependências para evitar erro de FK (se não tiver cascade)
    ...
    <div className="space-y-6">
      ...
      <div className="grid gap-2 sm:gap-4">
        {filteredPatients.map((patient) => {
            // LÓGICA PARA MOSTRAR TODOS OS RETORNOS
            // Filtra agendamentos futuros que sejam 'Agendado' ou 'Confirmado'
                ...
                <CardContent className="p-3 sm:p-4">
                  <div className="flex items-start justify-between gap-2 sm:gap-4">
                    <div className="flex-1 min-w-0">
                      ...
                      {/* AQUI MOSTRAMOS TODOS OS RETORNOS */}
...
function PatientModal({ open, onClose, patient, onSave, isLoading }) {
  const [formData, setFormData] = useState({ full_name: '', phone: '', whatsapp: '', email: '', birth_date: '', gender: '', cpf: '', address: '', origin: '', protocol: '', notes: '' });
  ...
  useEffect(() => {
    if (patient) {
      ...
      setFormData({ full_name: '', phone: '', whatsapp: '', email: '', birth_date: '', gender: '', cpf: '', address: '', origin: '', protocol: '', notes: '' });
    }
  }, [patient, open]);
