...
import { Plus, Edit2, Trash2, DollarSign, TrendingDown, TrendingUp, Calendar, CheckCircle2, CreditCard } from 'lucide-react';
...
export default function Financial() {
  ...
  const totalRevenue = filteredAppointments.reduce((sum, a) => sum + (a.final_value || 0), 0) + filteredInstallments.filter(i => i.paid).reduce((sum, i) => sum + (i.value || 0), 0);
  ...
  const getChartData = () => {
    ...
    for (let i = 0; i < 12; i++) {
        ...
        const rev = appointments.filter(a => isWithinInterval(new Date(a.date), { start: mStart, end: mEnd }) && a.status === 'Realizado').reduce((s, a) => s + (a.final_value || 0), 0);
        const exp = expenses.filter(e => isWithinInterval(new Date(e.due_date), { start: mStart, end: mEnd })).reduce((s, e) => s + (e.amount || 0), 0);
        data.push({ name: format(mStart, 'MMM'), faturamento: rev, despesas: exp });
    }
    return data;
  };
  ...
  return (
    <div className="space-y-6">
      ...
      <Tabs defaultValue="overview">
        <TabsList className="bg-stone-100"><TabsTrigger value="overview">Visão Geral</TabsTrigger><TabsTrigger value="expenses">Despesas</TabsTrigger><TabsTrigger value="installments">Parcelas</TabsTrigger></TabsList>
