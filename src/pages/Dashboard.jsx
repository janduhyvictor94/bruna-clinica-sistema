...
import { startOfMonth, endOfMonth, isWithinInterval, addDays, startOfDay, isSameDay } from 'date-fns';
...
export default function Dashboard() {
  ...
  // --- NOVA LÓGICA DE RETORNOS (CAPTURA TUDO) ---
  const getUpcomingReturns = () => {
    const today = startOfDay(new Date());
    const limit = addDays(today, 30); // Olha para os próximos 30 dias
    const alerts = [];

    patients.forEach(p => {
        // 1. Verifica data principal
        if (p.next_return_date) {
            const date = new Date(p.next_return_date);
            // Ajuste de fuso horário simples (adiciona 12h para garantir o dia correto)
            date.setHours(12);
            if (date >= today && date <= limit) {
                alerts.push({
                    ...
                    phone: p.phone,
                    date: p.next_return_date,
                    type: 'Principal',
                    desc: 'Retorno'
                });
            }
        }
        // 2. Verifica datas adicionais (Array)
  ...
  return (
    <div className="space-y-6 sm:space-y-8">
      ...
      <div className="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-3 gap-4 sm:gap-6">
        ...
        {/* CARD DE RETORNOS (Corrigido para mostrar TUDO) */}
        <Card className="bg-white border-stone-200 shadow-sm sm:col-span-2 lg:col-span-1">
          <CardHeader className="pb-2 p-4 sm:p-6 sm:pb-2 flex flex-row items-center justify-between">
            <CardTitle className="text-xs sm:text-sm font-semibold text-stone-900 flex items-center gap-2">
              <Bell className="w-3 h-3 sm:w-4 sm:h-4" /> Próximos Retornos (30 dias)
            </CardTitle>
          </CardHeader>
                ...
                {upcomingReturns.map((p) => (
                  <div key={p.id} className="flex items-center justify-between p-2 sm:p-3 bg-amber-50 rounded-xl gap-2 border border-amber-200">
                    <div className="min-w-0 flex-1">
                      <p className="text-xs sm:text-sm font-medium text-stone-900 truncate">{p.full_name}</p>
                      <p className="text-[10px] sm:text-xs text-stone-600">{p.phone} • {p.desc}</p>
                    </div>
                    <Badge variant="outline" className={`text-[10px] sm:text-xs flex-shrink-0 font-medium ${p.type === 'Principal' ? 'bg-amber-100 text-amber-800 border-amber-300' : 'bg-blue-50 text-blue-700 border-blue-200'}`}>
