...
function AppointmentModal({ open, onClose, appointment, patients, procedures, materials, allAppointments, onSave, isLoading }) {
  const [formData, setFormData] = useState({
    patient_id: '', patient_name: '', date: format(new Date(), 'yyyy-MM-dd'), time: '', status: 'Agendado',
  ...
  useEffect(() => { 
      if (appointment) {
          ...
          setFormData({ patient_id: '', patient_name: '', date: format(new Date(), 'yyyy-MM-dd'), time: '', status: 'Agendado', notes: '', next_return_date: '', scheduled_returns: [], is_new_patient: false, procedures_performed: [], materials_used: [], total_value: 0, total_material_cost: 0, payment_method: '', discount_percent: 0, discount_value: 0, final_value: 0, installments: 1, installment_value: 0 }); 
      }
  }, [appointment, open]);

  const handlePatientChange = (pid) => { const p = patients.find(pat => pat.id === pid); const hasApps = allAppointments.some(a => a.patient_id === pid); setFormData(prev => ({ ...prev, patient_id: pid, patient_name: p?.full_name, is_new_patient: !hasApps })); };
  ...
  return (
    <Dialog open={open} onOpenChange={onClose}>
      <DialogContent className="max-w-3xl max-h-[90vh] overflow-y-auto">
        <DialogHeader>
            <DialogTitle>{appointment ? 'Editar' : 'Novo'} Atendimento</DialogTitle>
            <DialogDescription className="hidden">Preencha os dados do atendimento</DialogDescription>
        </DialogHeader>
        <form onSubmit={handleSubmit} className="space-y-6">
          <div className="grid grid-cols-2 gap-4"><div className="col-span-2"><Label>Paciente</Label><Select value={formData.patient_id?.toString() || ''} onValueChange={v => handlePatientChange(parseInt(v))}><SelectTrigger><SelectValue placeholder="Selecione..."/></SelectTrigger><SelectContent>{patients.map(p => <SelectItem key={p.id} value={p.id.toString()}>{p.full_name}</SelectItem>)}</SelectContent></Select></div><div><Label>Data</Label><Input type="date" value={formData.date || ''} onChange={e => setFormData({...formData, date: e.target.value})} required/></div><div><Label>Hora</Label><Input type="time" value={formData.time || ''} onChange={e => setFormData({...formData, time: e.target.value})}/></div><div><Label>Status</Label><Select value={formData.status || 'Agendado'} onValueChange={v => setFormData({...formData, status: v})}><SelectTrigger><SelectValue/></SelectTrigger><SelectContent><SelectItem value="Agendado">Agendado</SelectItem><SelectItem value="Confirmado">Confirmado</SelectItem><SelectItem value="Realizado">Realizado</SelectItem><SelectItem value="Cancelado">Cancelado</SelectItem></SelectContent></Select></div><div><Label>Retorno</Label><Input type="date" value={formData.next_return_date || ''} onChange={e => setFormData({...formData, next_return_date: e.target.value})}/></div></div>
          <div className="p-4 bg-stone-50 rounded"><Label>Retornos Adicionais</Label><div className="flex gap-2"><Input type="date" value={newReturn.date} onChange={e => setNewReturn({...newReturn, date: e.target.value})} className="flex-1"/><Input placeholder="Desc" value={newReturn.description} onChange={e => setNewReturn({...newReturn, description: e.target.value})} className="flex-1"/><Button type="button" onClick={() => { if(newReturn.date) setFormData(prev => ({...prev, scheduled_returns: [...prev.scheduled_returns, newReturn]})); setNewReturn({date:'',description:''}); }}><Plus className="w-4 h-4"/></Button></div>{formData.scheduled_returns?.map((ret, i) => <div key={i} className="flex gap-2 bg-white p-2 mt-2 border rounded"><Calendar className="w-4 h-4 text-stone-400"/> {format(new Date(ret.date), 'dd/MM')} - {ret.description}</div>)}</div>
